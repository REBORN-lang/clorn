...
/*
 * clorn.c - reborn to c23 transpiler
 * implements revision 26015 of the reborn standard (RS)
 * supports R16 extension 1 (colon multi-instruction braceless blocks)
 *
 * pipeline: source -> lexer -> tokens -> parser -> ast -> codegen -> c23
 */
...
/* R16 extension 1: colon-body multi-instruction braceless block.
 * Called after consuming the ':' that follows a condition.
 * Reads statements until it hits elif/else/EOF or a token that
 * cannot start a statement (i.e. TK_RBRACE at depth 0).
 * This extension is always enabled in clorn (no flag required). */
static int tok_starts_stmt(TokenKind k) {
    switch (k) {
        case TK_LET: case TK_RETURN: case TK_IF: case TK_ELIF:
        case TK_ELSE: case TK_WHILE: case TK_FOR: case TK_EXTERN:
        case TK_IMPORT: case TK_FROM: case TK_LBRACE:
        case TK_IDENT: case TK_INT_LIT: case TK_FLOAT_LIT:
        case TK_STRING_LIT: case TK_CHAR_LIT: case TK_BOOL_LIT:
        case TK_PLUS: case TK_MINUS: case TK_NOT: case TK_STAR:
        case TK_BITAND: case TK_AT: case TK_INC: case TK_DEC:
        case TK_LPAREN: case TK_SIZEOF: case TK_TYPEOF:
            return 1;
        default:
            return 0;
    }
...
static Node *parse_colon_body(Parser *p) {
    int line = p_peek(p)->line;
    Node *blk = node_new(ND_BLOCK, line);
    /* read statements until we can't — stop before elif/else/} /EOF */
    while (!p_check(p, TK_EOF) &&
           !p_check(p, TK_RBRACE) &&
           !p_check(p, TK_ELIF) &&
           !p_check(p, TK_ELSE) &&
           tok_starts_stmt(p_peek(p)->kind)) {
        Node *s = parse_stmt(p);
        if (s) nodelist_push(&blk->stmts, s);
    }
    return blk;
...
/* unified body parser: handles {}, single-stmt braceless, and : colon body */
static Node *parse_body(Parser *p) {
    if (p_check(p, TK_LBRACE))  return parse_block(p);
    if (p_match(p, TK_COLON))   return parse_colon_body(p);
    /* plain braceless single statement */
    int line = p_peek(p)->line;
    Node *blk = node_new(ND_BLOCK, line);
    Node *s = parse_stmt(p);
    if (s) nodelist_push(&blk->stmts, s);
...
static void gen_node(CGen *g, Node *n) {
    ...
    switch (n->kind) {
        ...
        case ND_PROGRAM: {
            emit(g, "/* generated by clorn (reborn revision 26015) — target: ISO C23 */\n");
